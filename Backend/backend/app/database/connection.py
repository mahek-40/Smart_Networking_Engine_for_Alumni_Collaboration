...
async def create_indexes() -> None:
    ...
    Indexes created:
      ...
      - connections         → compound indexes for fast lookups
      - notifications       → user_id + read status
      - projects            → owner_id and status
    """
    db = get_database()
    if db is None:
    ...
    try:
        ...
        # Connections indexes
        await db["connections"].create_index("requester_id", background=True)
        await db["connections"].create_index("target_id", background=True)
        await db["connections"].create_index("status", background=True)
        await db["connections"].create_index(
            [("requester_id", ASCENDING), ("target_id", ASCENDING)],
            unique=True,
            background=True,
        )
        ...
        # Projects indexes
        await db["projects"].create_index("owner_id", background=True)
        await db["projects"].create_index("status", background=True)
        await db["projects"].create_index("members", background=True)
        await db["projects"].create_index("applicants", background=True)
